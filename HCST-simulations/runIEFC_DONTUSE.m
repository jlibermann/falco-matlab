...
mp.path.ws = '/media/Data_Drive/KPIC/dev/jliberman/FALCO_Repo/falco-matlab/data/iefc_smf/example_data/'; % (Mostly) complete workspace from end of trial. Default is [mp.path.falco filesep 'data' filesep 'ws' filesep];
...
mp.jac.fn = 'jac_iefc_test.mat'; %'jac_iefc_test.mat'; % Name of the Jacobian file to save or that is already saved. The path to this file is set by mp.path.jac.
...
% Overwrite DM params
mp.dm_ind = [1 2]; % Specify which DM to use
...
mp.runLabel = ['Series',num2str(mp.SeriesNum,'%04d'),'_Trial',num2str(mp.TrialNum,'%04d_'), '_Itrs',num2str(mp.Nitr,'%04d_'),...
    mp.coro,'_',num2str(numel(mp.dm_ind)),'DM',num2str(mp.dm1.Nact),'_z',num2str(mp.d_dm1_dm2),...
    '_IWA',num2str(mp.Fend.corr.Rin),'_OWA',num2str(mp.Fend.corr.Rout),...
    '_',num2str(mp.Nsbp),'lams',num2str(round(1e9*mp.lambda0)),'nm_BW',num2str(mp.fracBW*100),...
    '_SubBP', num2str(mp.Nsbp), mp.controller, '_DM', num2str(mp.dm_ind)];

%% Step 7: Perform the Wavefront Sensing and Control in FALCO
