...
% mp.thput_eval_x = 1; % x location [lambda_c/D] in dark hole at which to evaluate throughput
...
% mp.source_x_offset_norm = 1;  % x location [lambda_c/D] in dark hole at which to compute intensity normalization
...
mp.dm2.basisType = 'fourier';  % (UNCOMMENT FOR IEFC) basis set for control. 'actuator' or 'fourier' 
...
mp.Fend.corr.Rout  = 1;  % outer radius of dark hole correction region [lambd
...
mp.Fend.score.Rout = 1;  % outer radius of dark hole scoring region [lambda0/D]
% mp.Fend.score.Rout = 3; % Josh: Changed outer radius of scoring region
...
mp.Fend.xiOffset = mp.Fend.x_fiber; % Offset DH such that it overlaps fiber pos
mp.Fend.etaOffset = mp.Fend.y_fiber; % Offset DH such that it overlaps fiber pos
...
% mp.P2.D = (mp.dm1.Nact-2)*mp.dm1.dm_spacing; % Josh: changed pupil plane diameter
mp.P3.D = mp.P2.D;
mp.P4.D = mp.P2.D;
...
% 
% % % Debugging: Plot Z map on pupil
% imagesc(ZmapFull_norm.*mp.P1.full.mask)
% 
% % % Aberrated E-fields
% 
% % TODO: FIgure out what size is for 'sz'
% mp.P1.compact.E = zeros(size(ZmapFull, 1), size(ZmapFull, 2),mp.Nsbp);
% mp.P1.full.E = zeros(size(ZmapCompact, 1), size(ZmapCompact, 2), mp.Nwpsbp, mp.Nsbp);
% for II = 1:mp.Nsbp
%     mp.P1.compact.E(:,:,II) = exp(1j * 2 * pi / mp.lambda0 * zern_error_rms * ZmapCompact_norm);
%     for JJ = 1:mp.Nwpsbp
%         mp.P1.full.E(:,:,JJ, II) = exp(1j * 2 * pi / mp.lambda0 * zern_error_rms * ZmapFull_norm);
%     end
% end
